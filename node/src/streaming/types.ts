...
export interface TensorStreamConfig {
  ...
  metadata?:
      ...
      | ((tensor: MLXArray, index: number) => Record<string, unknown> | undefined);
  endMetadata?:
      | Record<string, unknown>
      | ((tensor: MLXArray, index: number) => Record<string, unknown> | undefined);
}
...
export type StreamValue =
  ...
  | string
  | MLXArray
  | Promise<SSEMessage | TensorStreamFrame | string | MLXArray>
  | AsyncIterable<StreamValue>
  | Iterable<StreamValue>;

export type StreamSource<T = StreamValue> =
  | AsyncIterable<T>
  | Iterable<T>
