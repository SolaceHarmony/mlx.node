...
export async function* tensorToFrames(
  tensor: MLXArray,
  options?: TensorFrameOptions,
  index: number = 0,
): AsyncIterable<TensorStreamFrame> {
...
export async function* tensorsToFrameStream(
  tensors: AsyncIterable<MLXArray> | Iterable<MLXArray>,
  options?: TensorFrameOptions,
): AsyncIterable<TensorStreamFrame> {
  let index = 0;
  if (Symbol.asyncIterator in tensors) {
    for await (const tensor of tensors as AsyncIterable<MLXArray>) {
      for await (const frame of tensorToFrames(tensor, options, index++)) {
        yield frame;
      }
    }
  } else {
    for (const tensor of tensors as Iterable<MLXArray>) {
      for await (const frame of tensorToFrames(tensor, options, index++)) {
        yield frame;
      }
    }
